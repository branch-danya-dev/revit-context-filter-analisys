...
**ContextFilter — не учебная система. Это реальный плагин для Autodesk Revit, который прошёл путь от запроса заказчика до разработки, пользовательского тестирования, приёмки и внедрения.**
...
Название организации намеренно не раскрывается. Точное количество пользователей и подразделений не заявляется, поскольку такие данные не собирались как часть кейса.
